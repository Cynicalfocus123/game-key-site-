...
export async function registerAndVerify(page: Page, { admin = false, name = "Test User", email = uniqueEmail("user") } = {}) {
  await page.goto(admin ? "admin/register/" : "register/");
  await page.locator("input[name=name]").fill(name);
  await page.locator("input[name=email]").fill(email);
  await page.locator("input[name=password]").fill(DEMO_PASSWORD);
  await page.locator("input[name=confirm]").fill(DEMO_PASSWORD);
  if (!admin) await page.getByRole("checkbox").first().check();
  await page.getByRole("button", { name: admin ? "Create admin account" : "Create account" }).click();
