...
## Handoff v10 (2026-09-26, mid Part 2) — latest, use this one

PROJECT: `D:\mstar companies\Game keys and ecommerce pc site` (D: only), mirror `live/` (byte-identical except test-only files `e2e/`, `scripts/e2e.mjs`, `scripts/serve-out.mjs`, `playwright.config.ts`, `test-results/`, `playwright-report/`). GitHub https://github.com/Cynicalfocus123/game-key-site- (`main`, push deploys Pages demo after Playwright).
READ FIRST: `CLAUDE.md`, `D:\dev\claude\CLAUDE.md`, this file (Handoff v8 = spec C1–C12 + PARTS; PROGRESS lines under it), `design.md`, `code.md` (section "Customer dashboard step 1"), `weight.md`. Confirm git clean at `94b02b1` or later and `live/` matches.
DONE THIS SESSION: breadcrumb fix (`4ddc1de`: global `nav` CSS scoped to `header nav`, `.crumbs` own style). Part 2 step 1 (`94b02b1`): dashboard sidebar + mobile section select, Overview (profile completion, balance display, Recent purchases), Login history (`/api/account/logins`), Orders table, Settings profile (avatar colour, country, currency, deals choice), migration `0005_profile`, placeholders `/account/balance`, `/account/keys`, `/account/tickets`. Tests 54 passed, 6 skipped. All pushed; nothing unpushed.
NEXT: Part 2 STEP 2 — keys library + key detail (C5–C8): per-key records (order item keys, `revealed_at`, reveal audit; server table + demo store), APIs; replace `/account/keys` placeholder (search, filter All/Not revealed/Revealed, table, 20 per page); `/account/keys/view?id=` (facts, masked key, Reveal key ends refund window, copy, Activate on {platform}, Print as a gift, My library, Sold by CoreCart, Report a problem → tickets placeholder); print page (C7); `/help/activate/{steam,xbox,playstation,nintendo,ea,ubisoft}` (C8). Then point Overview Recent purchases links (currently `/account/keys`) to `/account/keys/view?id=` and Orders detail key items to key detail (remove `KeyReveal` in `app/account/orders/page.tsx`); `OrderItem.revealedAt` type already exists. Then STEP 3 balance + gift cards, STEP 4 tickets (4a customer, 4b admin; wire `AccountShell unread` prop), STEP 5 polish + `/code-review`.
OPEN: user never answered the cut-off question ("showing the exact item and…"). Server mode not run at runtime for cart API, profile update, logins API (typecheck only; needs localhost permission).
TOOLING NOTES (new): Bash heredocs with `'EOF'` sometimes fail on long CSS — write to scratchpad with Write tool, then `cat >>`. Several files are CRLF: node string replace must normalise `\r\n` or use Edit tool. Screenshot checks: temp Playwright config + spec in Git-ignored `Claude outputs/shots-src/` (outputDir `D:/dev/tmp/pw-out`; `test-results/` gets wiped by test runs). `getByLabel("Account section")` also matches nav "Account sections" — use `getByRole("combobox", { name: "Account section" })`. `.cover` class belongs to `cart.css` (use `.thumb` in dashboard); global `footer` rule styles any `<footer>` like the site footer.
KNOWN ISSUES: v9 list unchanged (drawer "Under $10" not converted; admin user detail sums totals across currencies; `mxn.svg` 85 KB; unescaped `user.name` in email HTML; Google sign-up sets `termsAcceptedAt` without consent; Stripe routes lack try/catch; runtime migrations may race on serverless; wrong placeholder images; `#` links; Geist via CSS `@import`; server cart API not run; popup order does not move re-added items; "Keep me signed in" on register no effect until first sign-in). New: order items have no product id, so covers match catalog by name (`coverFor`); Xbox Game Pass shows a lettered tile.
RULES: never delete files unless told; no installs unless asked; no heavy deps; no localhost unless asked (Playwright approved); show side diff pane after every edit + `git -c color.ui=always --no-pager diff`; after every step: Playwright desktop + mobile, update 4 docs, sync `live/` (byte-verify), commit, push `main`, PROGRESS line under Handoff v8; caveman terse; no prompt suggestions or question cards (plain text); estimate context each step; handoff at end of `agents.md` before 250k tokens.
